...
SELECT f.name AS fac_name, CONCAT( m.firstname,  " ", m.surname ) AS mem_name, 
    CASE WHEN b.memid =0
    THEN f.guestcost * b.slots
    ELSE f.membercost * b.slots END AS cost
FROM Bookings b
JOIN Members m
JOIN Facilities f ON m.memid = b.memid AND f.facid = b.facid
WHERE b.starttime >  '2012-09-14' AND b.starttime <  '2012-09-15' AND (b.memid =0OR f.membercost * b.slots >30)
ORDER BY 3 DESC 
...
SELECT sub.fac_name,
       sub.mem_name,
       sub.cost
FROM (SELECT f.name AS fac_name, CONCAT(m.firstname, ' ', m.surname) AS mem_name,
...
SELECT sub.fac_name,
       sub.revenue
FROM (SELECT f.name AS fac_name,
             SUM(CASE WHEN b.memid = 0 THEN f.guestcost * b.slots ELSE f.membercost * b.slots END) AS revenue
      FROM (SELECT memid, facid, slots FROM Bookings) b
      JOIN (SELECT name, facid, membercost, guestcost FROM Facilities) f
      ON f.facid = b.facid
      GROUP BY 1) sub
